...
class TestPassGroundingAndCitations:
    ...
    async def test_minimal_passing_scenario(self, mock_get_adapter):
        ...
        # But stones_alignment is 0.5, not > 0.5, so no bonus
        # Score = 1 (base) + 1 (citations) = 2
        # Normalized = (2-1)/4 = 0.25, which is >= 0.25 threshold
        expected_grounding_1to5 = 2  # Base + citations (stones_alignment is exactly 0.5, not > 0.5)
